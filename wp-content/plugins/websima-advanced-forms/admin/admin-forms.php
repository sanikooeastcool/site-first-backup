...
class AF_Admin_Forms {
	...
	function fields_meta_box_callback() {
				...
				<?php if ( ! empty( $field_groups ) ) : ?>
					...
					<?php foreach ( $field_groups as $field_group ) : ?>
						...
						<tr class="field-group-heading">
							<td colspan="3">
								<a href="<?php echo get_edit_post_link( $field_group['ID'] ).'&from=form_builder&form_id='.$post->ID; ?>"><?php echo __( 'Edit', 'advanced-forms' ).' "'.$field_group['title'].'"'; ?></a>
							</td>
						</tr>
		...
		<a href="<?php echo admin_url( 'post-new.php?post_type=acf-field-group&from=form_builder&form_id='.$post->ID ); ?>" class="button">
	...
	function manage_columns( $columns ) {
		...
		$new_columns = array(
			...
			'shortcode' => __( 'Shortcode', 'advanced-forms' ),
			//'fields' 	=> __( 'Fields', 'advanced-forms' ),
			'entries' 	=> __( 'Entries', 'advanced-forms' ),
			'export' 	=> __( 'Export', 'advanced-forms' ),
		);
	...
	function custom_columns_content( $column, $post_id ) {
		...
		if ( 'key' == $column ) {
            ...
            echo sprintf( '<code>[advanced_form form="%s"]</code>', get_post_meta( $post_id, 'form_key', true ) );

        }elseif('export' == $column){
            $entries = af_get_entries( $form['key'] );
            if(count( $entries )){
                echo '<a href="'.esc_url(wfb_find_page_id('includes/websima-form-builder-extension/wfb-export-entries.php')).'?form_id='.esc_attr($post_id).'" target="_blank">'.__( 'Download', 'advanced-forms' ).'</a>';
            }else{
                echo '-';
            }
        }
		...
		$settings_field_group = array (
			...
			'fields' => array (
				...
				array (
					...
					'label' => __( 'Description (for manager)', 'advanced-forms' ),
                ...
                array(
                    'key' => 'field_form_submit_button_label',
                    'label' => __( 'Submit button label', 'advanced-forms' ),
				...
				array(
                    'key' => 'field_form_captcha',
                    'label' => __( 'Captcha', 'advanced-forms' ),
                    'name' => 'form_captcha',
                    'type' => 'true_false',
                    'instructions' => '',
                    'required' => 0,
                ...
                array(
                    'key' => 'field_form_ajax_submission',
                    'label' => __( 'Ajax submission', 'advanced-forms' ),
                    'name' => 'form_ajax_submission',
                    'type' => 'true_false',
                    'instructions' => '',
                    'required' => 0,
                    'conditional_logic' => 0,
				...
				array (
                    ...
                    'conditional_logic' => array(
                        array(
                            array(
                                'field' => 'field_form_ajax_submission',
                                'operator' => '==',
                                'value' => '1',
                            ),
                        ),
                ...
                array(
                    'key' => 'field_form_redirect_url',
                    'label' => __( 'Redirect url', 'advanced-forms' ),
