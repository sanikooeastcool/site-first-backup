...
?>
    <main id="main" class="site-main">
        <div class="container">            
            ...
            <?php
            $sliders1 = get_field('slider_shop' ,$term->taxonomy.'_'.$term->term_id);
            $sliders2 = get_field('slider_shop' ,$shop_page_id);
            if (!is_shop()){
                if($sliders1 != null) {
                    ?>
                    <div class="slider-shop-wrap">
                        <div class="slider-shop owl-carousel">
                            <?php foreach ($sliders1 as $slider1) {
                                $img = $slider1['image'];
                                if (wp_is_mobile()){
                                    if ($slider1['image_mobile'] != ''){
                                        $img = $slider1['image_mobile'];
                                    }
                                }
                                ?>
                                <div class="slides">
                                    <?php echo ($slider1['link']) ? '<a href="'.$slider1['link'].'">' : '<div>'; ?>
                                    <?php echo wp_get_attachment_image($img, 'full'); ?>
                                    <?php echo ($slider1['link']) ? '</a>' : '</div>'; ?>
                                </div>
                                <?php
                            } ?>
                        </div>
                ...
                if($sliders2 != null){
                    ...
                    <div class="slider-shop-wrap">
                        <div class="slider-shop owl-carousel">
                            <?php foreach ($sliders2 as $slider2) {
                                $img = $slider2['image'];
                                if (wp_is_mobile()){
                                    if ($slider2['image_mobile'] != ''){
                                        $img = $slider2['image_mobile'];
                                    }
                                }
                                ?>
                                <div class="slides">
                                    <?php echo ($slider2['link']) ? '<a href="'.$slider2['link'].'">' : '<div>'; ?>
                                    <?php echo wp_get_attachment_image($img, 'full'); ?>
                                    <?php echo ($slider2['link']) ? '</a>' : '</div>'; ?>
                                </div>
                                <?php
                            } ?>
                        </div>
                ...
                <div class="slider-shop-wrap">
                    <div class="slider-shop owl-carousel">
                           <?php foreach ($sliders as $slider) {
                                ...
                                ?>
                                <div class="slides">
                                    <?php echo ($slider['link']) ? '<a href="'.$slider['link'].'">' : '<div>'; ?>
                                        <?php echo wp_get_attachment_image($img, 'full'); ?>
                                    <?php echo ($slider['link']) ? '</a>' : '</div>'; ?>
                                </div>
                                <?php
                            } ?>
                    </div>
