...
function websima_delivery_init(){
    if( function_exists('acf_add_options_page') ) {
        acf_add_options_sub_page(array(
            'page_title' 	=> 'زمان تحویل سفارش',
            'menu_title'	=> 'زمان تحویل سفارش',
...
if(get_field('active_delivery', 'option')):
function websima_delivery_before_order_notes($checkout){
    echo '<div class="delivery-wrapper">';
        ...
        echo '<h3 class="title">روز و ساعت تحویل سفارش</h3>';
        if($delivery_description){ echo '<p class="description">'.esc_html($delivery_description).'</p>'; }

        $holidays = array();
        if(have_rows('holidays', 'option')):
            while(have_rows('holidays', 'option') ): the_row();
                if(get_sub_field('date')){ $holidays[] = get_sub_field('date'); }
            endwhile;
        endif;
        ...
        $day_options = array('0'=>'انتخاب روز');
        $time_options = array('0'=>'انتخاب ساعت');
        for($i;$i<$j;$i++){
            ...
            if(!in_array($date , $holidays)){
                ...
                if(in_array(parsidate("l", strtotime("+".$i." day")),$business_days)){
                    ...
                    if($i == 0){
                        ...
                        if($today){
                            $day_options[esc_attr($value)] = 'امروز، ' . esc_html($name);
                        }else{
                            $j++;
                        }
                    }else{
                        if($i == 1){ $name = 'فردا، ' . esc_html($name); }
        ...
        woocommerce_form_field( 'delivery_day', array(
            ...
            'label'         => __('تعیین روز تحویل سفارش'),
            'required'    => true,
            'options'     => $day_options
        ),
        ...
        if($delivery_time_status == 1){
            woocommerce_form_field( 'delivery_time', array(
                ...
                'label'         => __('تعیین ساعت تحویل سفارش'),
                'required'    => true,
...
function websima_delivery_checkout_process(){
    if($_POST['delivery_day'] == 0){
        wc_add_notice(esc_html('روز تحویل سفارش را مشخص کنید.'), 'error');
    }

    if(get_field('delivery_time_status', 'option') == 1){
        if($_POST['delivery_time'] == 0){
            wc_add_notice(esc_html('ساعت تحویل سفارش را مشخص کنید.'), 'error');
        }
    }
}
endif;
...
function websima_delivery_admin_order_data_after_billing_address($order){
    ...
    if($delivery_day){
        echo '<p><strong>تاریخ تحویل سفارش: </strong><span style="display: inline-block;">'.esc_html($delivery_day).'</span></p>';
    }
    ...
    if($delivery_time){
        ...
        echo '<p><strong>ساعت تحویل سفارش: </strong><span>'.esc_html($final_time).'</span></p>';
    }
}
...
function websima_delivery_view_order_and_thankyou_page($order_id){
    ...
    if($delivery_day or $delivery_time){
        echo '<div class="delivery-wrap">';
            echo '<h2>روز و ساعت تحویل سفارش</h2>';
            echo '<ul>';
                 if($delivery_day){
...
function websima_delivery_time_options(){
    ...
    if(get_field('delivery_time_status', 'option') == 1){
        if($day){
            if(parsidate("Y-m-d", strtotime("+0 day") , 'en') == $day){
                ...
                if(have_rows('delivery_time', 'option')){
                    while(have_rows('delivery_time', 'option')){
                        ...
                        $end = get_sub_field('end');
                        if($available_time < $start){
                            $times[esc_attr($start).'-'.esc_attr($end)] = 'از '.esc_html($start).' تا '.esc_html($end);
                        }
                    }
                }
            }else{
            ...
            $status = 0;
            $error = 'ابتدا روز را انتخاب نمایید';
        }
    }else{
        $status = 0;
        $error = 'گزینه انتخاب زمان فعال نیست';
    }
