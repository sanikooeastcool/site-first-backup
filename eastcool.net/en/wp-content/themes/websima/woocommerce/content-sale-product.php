    ...
    <article <?php wc_product_class('product-item', $product); ?>>
        ...
        <a href="<?php the_permalink(); ?>" class="product-image" <?php if (isset($dataimg)) echo $dataimg; ?>>
            ...
            <?php if (get_field('is_new') || get_field('is_sale')) {
                ...
                if (get_field('is_Festival')) echo "<span class='on-Festival'>festival8</span>";
                echo "</div>";
            } ?>
            <?php
        ...
        <div class="product-body">
            <?php if (wc_product_sku_enabled() && ($product->get_sku() || $product->is_type('variable'))) : ?>
                <span class="sku_wrapper_card"><?php esc_html_e('', 'woocommerce'); ?> <span class="sku"><?php echo ($sku = $product->get_sku()) ? $sku : esc_html__('N/A', 'woocommerce'); ?></span></span>
            <?php endif; ?>
            <h2><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
            <?php
            if (!$product->is_in_stock()) {
                echo '<p class="stock out-of-stock">unavailable</p>';
            } else { ?>
