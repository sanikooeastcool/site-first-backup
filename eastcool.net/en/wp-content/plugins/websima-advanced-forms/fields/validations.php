        ...
        if (strlen($value) == '11') {
            if (is_numeric($value)) {
                if (preg_match('/^0[1-9]{1}[0-9]{9}$/', $value)) {
                    ...
                    return esc_html($field['label']) . 'Entered is not valid';
                }
            } else {
                return esc_html($field['label']) . 'Must be entered numerically';
            }
        } else {
            return 'length ' . esc_html($field['label']) . 'Must be 11 characters';
        }
    }
}
