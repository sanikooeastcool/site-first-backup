                ...
                if(get_post_type(esc_attr($form_id)) == 'af_form'){
                    ...
                    if(have_rows('manager_notify_email_repeater', esc_attr($form_id))):
                        while(have_rows('manager_notify_email_repeater', esc_attr($form_id))): the_row();
                            ...
                            $subject = 'ورودی - '.get_the_title($form_id);
                            $body = 'سلام فرم جدیدی در سایت پر شده است، لطفا صندوق ورودی سایت را بررسی نمایید.';
...
function wfb_admin_menu(){
    if( function_exists('acf_add_options_page') ) {
        acf_add_options_sub_page(array(
            'page_title' 	=> 'تنظیمات فرم ساز',
            'menu_title'	=> 'تنظیمات فرم ساز',
    ...
    acf_add_local_field_group(array(
        ...
        'fields' => array(
            ...
            array(
                ...
                'label' => 'الگوی پیامک اعلان تکمیل فرم برای مدیر',
                'name' => 'wfb_submission_manager_notify_template',
                'type' => 'text',
                'instructions' => 'سلام %token10 عزیز فرم جدیدی در سایت پر شده است، لطفا صندوق ورودی سایت را بررسی %token. فروشگاه وبسیما',
                'required' => 1,
                'conditional_logic' => 0,
                'wrapper' => array(
                    'width' => '',
            ...
            array(
                ...
                'hide_admin' => 0,
                'message' => 'برای دریافت پیامک پس از تکمیل فرم، باید یک فیلد از نوع متنی با نام user_mobile در فرم ایجاد نمایید. در صورتی که کاربر این فیلد را تکمیل کرده باشد پیامک به کاربر ارسال خواهد شد.
                برای دریافت ایمیل پس از تکمیل فرم، باید یک فیلد از نوع پست الکترونیک با نام user_email در فرم ایجاد نمایید. در صورتی که کاربر این فیلد را تکمیل کرده باشد ایمیل به کاربر ارسال خواهد شد.
                ',
                'new_lines' => 'wpautop',
                'esc_html' => 0,
            ),
            array(
                ...
                'label' => 'الگوی پیامک اعلان تکمیل فرم برای کاربر',
                'name' => 'wfb_submission_user_notify_template',
                'type' => 'text',
                'instructions' => 'سلام پیام شما دریافت %token. فروشگاه وبسیما',
                'required' => 1,
                'conditional_logic' => 0,
                'wrapper' => array(
                    'width' => '',
...
function wfb_entry_status_translator($status){
    $status_str = '';
    if($status == 'pending'){
        $status_str = 'در انتظار بررسی';
    }elseif($status == 'need_to_follow'){
        $status_str = 'نیاز به پیگیری';
    }elseif($status == 'checked'){
        $status_str = 'بررسی شده';
    }else{
        $status_str = ' - ';
    }
...
function wfb_add_page_templates( $post_templates, $wp_theme, $post, $post_type ) {
    $post_templates['includes/websima-form-builder-extension/wfb-print-entry.php'] = 'چاپ پیام ورودی';
    $post_templates['includes/websima-form-builder-extension/wfb-export-entries.php'] = 'برون ریزی فرم';
...
function wfb_validate_form($form,$args){
	if(get_post_meta(esc_attr($form['post_id']),'form_captcha',true)){
		...
		if($c4wp_empty_messages == ''){ $c4wp_empty_messages = 'عبارت امنیتی را وارد نمایید'; } 
		if($c4wp_errors_messages == ''){ $c4wp_errors_messages = 'عبارت امنیتی صحیح را وارد نمایید'; } 
