...
final class ITSEC_Admin_Page_Loader {
	...
	public function add_admin_pages() {
		...
		$page_refs[] = add_submenu_page( $parent, __( 'iThemes Security Settings', 'it-l10n-ithemes-security-pro' ), $onboarded ? __( 'Settings', 'it-l10n-ithemes-security-pro' ) : __( 'Setup', 'it-l10n-ithemes-security-pro' ), $capability, 'itsec', array( $this, 'show_page' ) );

		$page_refs = apply_filters( 'itsec-admin-page-refs', $page_refs, $capability, array( $this, 'show_page' ), $parent );

		if ( $onboarded ) {
			$page_refs[] = add_submenu_page( $parent, __( 'iThemes Security Logs', 'it-l10n-ithemes-security-pro' ), __( 'Logs', 'it-l10n-ithemes-security-pro' ), $capability, 'itsec-logs', array( $this, 'show_page' ) );
		}

		if ( ! ITSEC_Core::is_pro() ) {
			$page_refs[] = add_submenu_page( $parent, '', '<span style="color:#2EA2CC">' . __( 'Go Pro', 'it-l10n-ithemes-security-pro' ) . '</span>', $capability, 'itsec-go-pro', array( $this, 'show_page' ) );
		}

		if ( defined( 'ITSEC_DEBUG' ) && ITSEC_DEBUG ) {
			$page_refs[] = add_submenu_page( $parent, __( 'iThemes Security Debug', 'it-l10n-ithemes-security-pro' ), __( 'Debug', 'it-l10n-ithemes-security-pro' ), $capability, 'itsec-debug', array( $this, 'show_page' ) );
		}
