...
class AF_Core_Forms_Rendering {
  ...
  function render( $form_id_or_key, $args ) {
    ...
    $submit_button_label = get_post_meta(esc_attr($form['post_id']),'form_submit_button_label',true);
    $ajax_submission = get_post_meta(esc_attr($form['post_id']),'form_ajax_submission',true);
    if($ajax_submission == 1){
        $ajax_mode = true;
        $form_redirect_url = NULL;
    }else{
        $ajax_mode = false;
        $form_redirect_url = get_post_meta(esc_attr($form['post_id']),'form_redirect_url',true);
    }
    ...
    $args = wp_parse_args($args, array(
      ...
      'id'            => $form['key'],
      'values'          => array(),
      'submit_text'         => esc_html($submit_button_label),
