...
function websima_newsletter_enqueue_scripts()
{
    ...
    wp_localize_script(
        'websima_newsletter-script',
        'newsletter_script_dyn_data',
        array(
...
{ ?>
    <form method="post" id="newsletter-form" novalidate="novalidate" class="footer-newsletter">
        ...
        <input type="email" name="email" id="email" class="form-control ltr" placeholder="please enter your email address">
...
function websima_newsletter_mailerlite($email, $group_id, $args = null)
{
    ...
    if (!is_wp_error($response)) {
        ...
        if ($response_code == 200) {
            ...
            $result['flag'] = 1;
            $result['message'] = 'mission accomplished';
        } elseif ($response_code == 401) {
            $result['flag'] = 0;
            $result['message'] = 'The API Key entered is invalid';
        } elseif ($response_code == 404) {
            $result['flag'] = 0;
            $result['message'] = 'The entered group is not valid';
        } else {
            $result['flag'] = 0;
        ...
        if ($dash_position !== false) {
            ...
            if (!is_wp_error($response)) {
                ...
                if ($response_code == 200) {
                    ...
                    $result['flag'] = 0;
                    $result['message'] = 'The given email has already been shared';
                } elseif ($response_code == 401) {
                    $result['flag'] = 0;
                    $result['message'] = 'The API Key entered is invalid';
                } elseif ($response_code == 404) {
                    $result['flag'] = 0;
                    $result['message'] = 'The list entered is invalid';
                } else {
                    $result['flag'] = 0;
                    $result['message'] = 'An error has occurred';
                }
