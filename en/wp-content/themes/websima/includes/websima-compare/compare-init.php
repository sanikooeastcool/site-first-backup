...
{
    if (get_option('compare_unique_id')) {
        $unique_id = get_option('compare_unique_id');
    } else {
        $unique_id = 'shop';
    }
...
{
    $post_templates['includes/websima-compare/compare.php'] = 'compare';
...
{
    echo '<div class="compare-btn" data-id="' . esc_attr($pid) . '" title="' . esc_attr(get_option('compare_tooltip_out')) . '">';
    echo '<i class="icon-comparison-list"></i>';
    echo '</div>';
}
