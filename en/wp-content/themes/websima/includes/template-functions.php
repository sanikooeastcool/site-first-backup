			...
			foreach ($faqs as $faq) :
				...
				if ($faq_items != null) {
					...
					if (is_user_logged_in() && current_user_can('administrator')) {
						echo '<p>There is no content for this section, enter the questions from the admin panel.</p>';
					} else {
						echo '<p>Unfortunately, there is no content for this section yet.</p>';
					}
				}
				echo '</div>';
				$j++;
...
if (!function_exists('websima_faq')) {
	function websima_faqs($term = null, $title = null)
	{
		...
		if ($title == 'true' && $faqs) {
			...
			echo '<h4 class="title-heading">FAQS</h4>';
			echo '</div>';
		}
		if ($faqs) {
			$j = 1;
		...
		if ($map_type == 'single') {
			...
			if (have_rows('branch_repeater')) :
				...
				while (have_rows('branch_repeater')) : the_row();
					...
					echo '<span>Follow us on social media</span>';
					websima_socials();
					echo '</div>';
					echo '</div>';
					echo '<div class="col-12 col-md-6">';
			...
			if (have_rows('branch_repeater')) :
				...
				while (have_rows('branch_repeater')) : the_row();
					...
					if ($show_google_dir || $show_waze) :
						echo '<span class="path--easy" >Easy routing</span>';
					endif;
					echo '</div>';
...
if (!function_exists('websima_contact_form')) {
	function websima_contact_form()
	{
		$setting = array(
			...
			'post_content' => false,
			'submit_value'	=> 'Send',
			'updated_message' => __("Thank you for your message, your message was sent successfully.", 'acf'),
			'html_submit_button'	=> '<button type="submit" class="acf-button button" value="%s">Send</button>',
			'new_post'		=> array(
				'post_type'		=> 'contact',
				'post_status'	=> 'draft'
			),
	...
	if (is_single()) {
		if (get_field('help_post', $post->ID)) {
			...
			if (is_array($finds)) {
				unset($finds[0]);
				$help_heading = '';
				$help_heading .= '<div class="help-heading"><span>What you read in this article</span><ul>';
				$i = 1;
